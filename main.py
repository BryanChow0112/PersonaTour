...
st.button('Dont click me!', on_click=no_click)
...
option = st.selectbox('What food do you like?', ('Pizza', 'Burger', 'Salad'))
...
def main():
  ...
  for i, row in enumerate(st.session_state.board):
    ...
    for j, field in enumerate(row):
      cols[j + 1].button(
          field,
          key=f"{i}-{j}",
          on_click=handle_click if st.session_state.player == 'X'
          or st.session_state.opponent == 'Human' else computer_player(),
          args=(i, j),
      )
  ...
  player.button(f'{"❌" if st.session_state.player == "X" else "⭕"}\'s turn'
